...
const paymentIntent = async (
  ...
  try {
    ...
    var cart = [];
    var cbUser = 0;
    async function getStripeAccountID(firebaseCompanyID: string) {
      var accountID = 0;
      await db
        .collection("Impresa")
        .doc(`${firebaseCompanyID}`)
        .get()
        .then((snapshot: { data: Function }) => {
    ...
    async function getCartFromFirebase(firebaseUserID: string, shopID: string) {
      const cart: any[] = [];
      const cartCollection = await db
        .collection(`Cart/${firebaseUserID}/${shopID}`)
        .get();
      cartCollection.forEach((doc: { id: string; data: Function }) => {
    ...
    async function getCartFromMongodb(cart: any[]) {
    ...
    async function getCbUser(firebaseUserID: string) {
      var cb = 0;
      try {
        await db
