...
const product = async (_, { id }: QueryProductArgs, { client }) => {
  try {
    //check if the product is cached than return it
    const redisProduct = await useGet(`product/${id}`, client);
    if (redisProduct) {
      if (redisProduct.status === "not_active") throw new Error("product is not active");
      return redisProduct;
    }
    ...
    if (product.status === "not_active") throw new Error("product is not active");
